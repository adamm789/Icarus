...
namespace Icarus.Util
{
    public class MdlWithLumina
    {
        ...
        private readonly Dictionary<VertexUsageType, VertexDataType> VertexDict = new();
        ...
        public static XivMdl GetRawMdlDataLumina(string mdlPath, bool getOriginal = false, long offset = 0)
        {
            XivMdl ret = new();
        ...
        public XivMdl GetRawMdlDataLumina(bool getOriginal = false, long offset = 0)
        {
            ...
            for (int i = 0; i < ImportedXivMdl.LoDList.Count; i++)
            {
                ...
                for (var j = 0; j < totalMeshCount; j++)
                {
                    ...
                    foreach (var ve in OriginalMdlFile.VertexDeclarations[idx].VertexElements)
                    {
                        ...
                        {
                            // TODO: See if existing keys match correctly?
                            /*if (vertexDict.Get(vertexDataStruct.DataUsage) != vertexDataStruct.DataType)
                            {
